#!/usr/bin/env bash
# Runs the Task #009 security regression suite
# (artifacts/api-server/src/security/security009.integration.mjs) against the
# real built server (dist/index.mjs), the same model as the Task #008 suite.
set -euo pipefail

name="nexora-task009-pg-$$"
port="${NEXORA_TEST_PG_PORT:-55436}"
apiport="${NEXORA_TEST_API_PORT:-4109}"
db="nexora_task009"
user="task009"
pass="task009-disposable-password"
...
docker run --rm --network host "${mounts[@]}" \
  ...
  -e JWT_SECRET=task009-disposable-jwt-secret-0123456789 \
  -e ENROLLMENT_SECRET=task009-disposable-enrollment-secret-0123456789 \
  -e ADMIN_API_TOKEN=task009-disposable-admin-token-0123456789 \
...
docker run --rm --network host "${mounts[@]}" \
  ...
  nexora-task010-integration bash -c '
    ...
    "
    node --test artifacts/api-server/src/security/security009.integration.mjs
  '
