...
# (artifacts/api-server/src/tenancy/pr05b-onboarding.integration.mjs) against the real
...
name="nexora-pr05b-pg-$$"
port="${NEXORA_TEST_PG_PORT:-55491}"
apiport="${NEXORA_TEST_API_PORT:-4181}"
db="nexora_pr05b"
user="pr05b"
pass="pr05b-disposable-password"
...
docker run --rm --network host "${mounts[@]}" \
  ...
  -e JWT_SECRET=pr05b-disposable-jwt-secret-0123456789 \
  -e ENROLLMENT_SECRET=pr05b-disposable-enrollment-secret-0123456789 \
  -e ADMIN_API_TOKEN=pr05b-disposable-admin-token-0123456789 \
...
docker run --rm --network host "${mounts[@]}" \
  ...
  nexora-task010-integration bash -c '
    ...
    "
    node --test artifacts/api-server/src/tenancy/pr05b-onboarding.integration.mjs
  '
