...
import { useOrganizationScope, useSession } from '@/lib/session';
...
export default function Devices() {
  ...
  // The selector only narrows the request. The API returns the caller's own
  // organizations regardless, so an organization user cannot see more by
  // omitting or forging this parameter.
  const showOrganizationColumn = Boolean(session?.platform_access) || (session?.organizations.length ?? 0) > 1;
  useEffect(() => { setPage(1); }, [search, status, organizationId]);
  const params: ListDevicesParams = { search: search || undefined, status: status === 'ALL' ? undefined : status, page, page_size: pageSize, ...(organizationId ? { organization_id: organizationId } : {}) };
