...
import { ArrowLeft, Bell, Boxes, ChevronRight, Cpu, Gauge, HardDrive, MonitorPlay, Network, RefreshCw, ServerCog, ShieldCheck, SquareActivity, UserRound, Wifi, Terminal, Power } from 'lucide-react';
...
import { DeviceRemoteDesktopControl } from '@/components/device-remote-desktop-control';
...
export default function DeviceDetail() {
  ...
  return <AppShell><div className="mb-4 flex items-center gap-2 text-[11px] text-muted-foreground"><Link href="/devices" className="inline-flex items-center gap-1 transition-colors hover:text-primary" data-testid="link-back-devices"><ArrowLeft size={13} /> Devices</Link><ChevronRight size={13} /><span className="font-mono-data text-primary">{device.hostname}</span></div><PageIntro eyebrow="Endpoint detail" title={device.hostname} description={`${device.os_name || 'Windows endpoint'}${device.os_build ? ` · ${device.os_build}` : ''} · Agent ${device.agent_version || 'version unavailable'}`} action={<div className="flex items-center gap-3"><Link href={`/devices/${device.id}/remote`} className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-3 py-2 text-[11px] font-semibold text-primary transition-colors hover:border-accent" data-testid="link-device-remote"><MonitorPlay size={14} />Remote</Link><StatusPill status={device.status} /><button type="button" onClick={() => { void deviceQuery.refetch(); void monitoringQuery.refetch(); }} className="rounded-md border border-border bg-card p-2 text-muted-foreground transition-colors hover:border-accent hover:text-primary" aria-label="Refresh endpoint" data-testid="button-refresh-device"><RefreshCw size={15} className={deviceQuery.isFetching ? 'animate-spin' : ''} /></button></div>} />
    ...
    {tab === 'overview' && <DeviceRemoteDesktopControl key={device.id} deviceId={device.id} organizationId={device.organization_id} />}
    {tab === 'overview' && <OverviewTab device={device} monitoring={monitoringQuery.data} onRefresh={() => void deviceQuery.refetch()} />}
...
function OverviewTab({ device, monitoring, onRefresh }: { device: DeviceDetails; monitoring?: MonitoringResponse; onRefresh: () => void }) {
...
function RemoteCommandsPanel({ device, onRefresh }: { device: DeviceDetails & { remote_commands_enabled?: boolean }; onRefresh: () => void }) {
  ...
  const setDeviceGate = async (enabled: boolean) => { await apiRequest(`/v1/devices/${device.id}/remote-commands`, { method: 'PATCH', body: JSON.stringify({ enabled }) }); onRefresh(); };
  const setGlobalGate = async (enabled: boolean) => { await apiRequest('/v1/remote-commands/gate', { method: 'PATCH', body: JSON.stringify({ enabled }) }); await gate.refetch(); };
  const submit = async (event: FormEvent) => { event.preventDefault(); setNotice(undefined); try { const result = await apiRequest<{ job: { id: string }; privileged_action_id: string }>('/v1/remote-commands', { method: 'POST', body: JSON.stringify({ device_id: device.id, shell, command, timeout_seconds: Number(timeout), reason }) }); setNotice(`PENDING_APPROVAL · Job ID ${result.job.id} · Privileged Action ID ${result.privileged_action_id}`); setDialogOpen(false); } catch (error) { setNotice(error instanceof Error ? error.message : 'Request failed'); } };
  if (!canManageDevice && !canRequest && !canManageGlobal) return null;
  const globalOn = gate.data?.enabled ?? false;
  const deviceOn = Boolean(device.remote_commands_enabled);
  return <><Panel className="mt-5"><PanelHeading eyebrow="Remote commands" title="Execution controls" meta="Two-person approval is always required" /><div className="grid gap-4 p-5 sm:grid-cols-2"><div className="rounded-md border border-border p-4"><div className="flex items-center justify-between"><div><p className="text-[11px] font-semibold text-primary">Global Remote Commands</p><p className="mt-1 text-[10px] text-muted-foreground">Status: {globalOn ? 'ON' : 'OFF'}</p></div><Power size={16} className={globalOn ? 'text-emerald-600' : 'text-muted-foreground'} /></div>{canManageGlobal && <div className="mt-3 flex gap-2"><button type="button" className="rounded-md border px-3 py-2 text-[10px] font-semibold" onClick={() => void setGlobalGate(true)} disabled={globalOn}>Enable</button><button type="button" className="rounded-md bg-destructive px-3 py-2 text-[10px] font-semibold text-destructive-foreground" onClick={() => void setGlobalGate(false)} disabled={!globalOn}>Disable Remote Commands</button></div>}</div><div className="rounded-md border border-border p-4"><div className="flex items-center justify-between"><div><p className="text-[11px] font-semibold text-primary">Device Remote Commands</p><p className="mt-1 text-[10px] text-muted-foreground">Status: {deviceOn ? 'ON' : 'OFF'}</p></div><Terminal size={16} className={deviceOn ? 'text-emerald-600' : 'text-muted-foreground'} /></div>{canManageDevice && <button type="button" className="mt-3 rounded-md border px-3 py-2 text-[10px] font-semibold" onClick={() => void setDeviceGate(!deviceOn)}>{deviceOn ? 'Disable' : 'Enable'}</button>}</div></div>{canRequest && globalOn && deviceOn && <div className="border-t border-border px-5 py-4"><button type="button" className="rounded-md bg-primary px-4 py-2 text-[11px] font-semibold text-primary-foreground" onClick={() => setDialogOpen(true)}>Run Command</button></div>}{notice && <p className="border-t border-border px-5 py-3 text-[11px] font-semibold text-primary">{notice} · <a className="underline" href="/security/approvals">Open approvals</a></p>}</Panel><Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent><DialogHeader><DialogTitle>Submit remote command</DialogTitle><DialogDescription>The command will remain pending until a different authorized person approves it.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-4"><label className="block text-[11px] font-semibold">Shell<select value={shell} onChange={(event) => setShell(event.target.value as 'CMD' | 'POWERSHELL')} className="mt-1 w-full rounded-md border p-2"><option value="CMD">CMD</option><option value="POWERSHELL">PowerShell</option></select></label><label className="block text-[11px] font-semibold">Command<textarea required rows={8} spellCheck={false} value={command} onChange={(event) => setCommand(event.target.value)} className="mt-1 min-h-40 w-full resize-y rounded-md border p-2 font-mono" /></label><label className="block text-[11px] font-semibold">Timeout (seconds)<input required type="number" min="1" max="900" value={timeout} onChange={(event) => setTimeoutValue(event.target.value)} className="mt-1 w-full rounded-md border p-2" /></label><label className="block text-[11px] font-semibold">Reason<textarea required value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1 w-full rounded-md border p-2" /></label><DialogFooter><button type="submit" className="rounded-md bg-primary px-4 py-2 text-[11px] font-semibold text-primary-foreground">Submit for Approval</button></DialogFooter></form></DialogContent></Dialog></>;
}
